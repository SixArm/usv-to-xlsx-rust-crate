...
use usv_to_xlsx::usv_to_xlsx_worksheet;
...
fn bench_example(c: &mut Criterion){
    ...
    let data = random_usv_data(unit_length, unit_count, record_count);

    let mut group = c.benchmark_group(
        &format!("benchmark group unit_length: {}, unit_count: {}, record_count: {}", unit_length, unit_count, record_count)
    );
    group.sample_size(10);
    group.bench_function("usv_to_xlsx_worksheet", |b| b.iter(|| usv_to_xlsx_worksheet(&data)));
    group.finish();
}
